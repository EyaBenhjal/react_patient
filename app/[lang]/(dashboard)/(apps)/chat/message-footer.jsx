...
const MessageFooter = ({ handleSendMessage, replay, setReply, replayData }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    handleSendMessage(message, messageType); // Include messageType
    setReply(false);
    setMessage("");
    setMessageType("text"); // Reset message type after sending
  };
    ...
    <>
      ...
      >
        <div className="flex-none flex gap-1 absolute md:static top-0 left-1.5 z-10">
          ...
          <Button
            type="button"
            className={`mr-2 rounded-full ${messageType === "text" ? "bg-primary text-white" : "bg-default-200"}`}
          ...
          <Button
            type="button"
            className={`mr-2 rounded-full ${messageType === "video" ? "bg-primary text-white" : "bg-default-200"}`}
              ...
              {messageType === "video" && (
                <div className="flex-1">
                  <input
                    ...
                    onChange={(e) => {
                      // Handle video file selection here
                    }}
                  />
                </div>
              )}
